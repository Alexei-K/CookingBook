...
open class StartInfoFragment : Fragment() {
    var viewModel: StartInfoViewModel? = null
    ...
    private fun setObservers(view: View) {
        viewModel?.image?.observe(viewLifecycleOwner, Observer { s: Int? ->
        ...
        viewModel?.title?.observe(viewLifecycleOwner, Observer { s: String? ->
            val mTitle = view.findViewById<TextView>(R.id.title_info)
            mTitle.text = s
        })
        viewModel?.text?.observe(viewLifecycleOwner, Observer { s: String? ->
